...
def testcase(f, Rar = 0.98,Radr = 0.999, Dyna = 0, maxiter = 500, verbose = False):
    inf = open(f)
    ...
    for iteration in range(0, maxiter): 
        ...
        if verbose: 
            printmap(data)
            print "#steps:  " , steps,"  iteration: ", iteration,  "iteration time: ", time.time() - st1
            print 
        #if verbose: time.sleep(1)
    et = time.time()
    printmap(data)
    print f
    print "#steps:  " , steps, "  time: ", et - st
    #print "------------------------------Transition Matrix---------------------------------"
    #for key, val in sorted(learner.model_s.iteritems()):
    #    if len(val) > 0:
    #        print key, ":  ", val
    #        assert np.all(np.array(val) == np.array(val)[0])
...
if __name__=="__main__":
    ...
    # read in the map
    infs = ['./testworlds/'+ inf for inf in sorted(os.listdir('./testworlds/'))]
    if len(sys.argv) == 2:
        if int(sys.argv[1]) == 0:
            for f in infs:
                print 
            ...
            #testcase('testworlds/world03.csv',verbose =  True)
            print "############################### Test Dyna-Q #############################################"
            testcase('testworlds/world03.csv', 0.5, 0.98, 200, 50, True)
        elif int(sys.argv[1]) == 2:
            for f in infs:
                print 
                print '--------------------------------------------------------------------------------'
                testcase(f, 0.5, 0.99, 200, 50, False)
        else:
            print "Please enter 0, 1, or 2 as argument to test no dyna, dyna on single file, dyna on all files"
    elif len(sys.argv) < 2:
        print "Please enter 0, 1, or 2 as argument to test no dyna, dyna on single file, dyna on all files"
    else:
